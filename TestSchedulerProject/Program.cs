    ...
    class Program
    {
        ...
        static private void runAlgo(int sessionId)
        {
            ...
            if (cursession.Id != 0)
            {
                ...
                foreach (var item in taskTypes.ToList())
                {
        ...
        static private Tuple<IEnumerable<PriceData>, IList<SMPR_testing_Lib.Domain.Task>> EvaluateManyOfManyQuestions(Session session, IQueryable<SMPR_testing_Lib.Domain.Task> tasks)
        {
            ...
            foreach (var student in session.Group.Users)
            {
				...
				if (answersForOneStudent.Count() > maxAnswersCount) {
					maxAnswersCount = answersForOneStudent.Count();
				}
            }
			...
			return RealTimeJudge.ManyOfManyQuestions.AnswersPriceEvaluator.EvaluateManyOfManyQuestions(tasks.ToList(), studAnswers.Where(x => x.Count() == maxAnswersCount).ToList(), marks, maxMarkForQuestionBlock);
        }
